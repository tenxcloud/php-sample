					...
					<?php foreach ( $list as $index => $val ) : ?>
						<?php if ( $val['count'] == 1 ): ?>
							<tr>
								...
								<?php else: ?>
									<td>
										<span class="label l-cur-step"><?php echo $index; ?></span>
									</td>
									<td>
										<span><?php echo $prcsName[$val['flowprocess']]; ?></span>
									</td>
								<?php endif; ?>
								<td><span class="fss"><?php echo $val['userName']; ?></span></td>
							</tr>
						<?php endif; ?>
					<?php endforeach; ?>
				</tbody>
	...
	$(function() {
		...
		$('#entrust_form').on('submit', function() {
			if (this.prcs_other.value == '') {
				Ui.tip('@WF.DESIGNATED_THE_PRINCIPAL', 'danger');
				return false;
			}
			return true;
		});
