                ...
                <div class="fill-zn">
                    ...
                    <div>
                        <?php foreach ( $fl as $key => $val ): ?>
                            <div class="well well-white media<?php if ( $val['flag'] == '2' ): ?> processing<?php endif; ?>">
                                <span class="label <?php if ( $val['flag'] == '2' ): ?>label-warning<?php else: ?>label-inverse<?php endif; ?> pull-left"><?php echo $key; ?></span>
                                <div class="media-body">
                                    <div class="media-heading">
                                        <?php if ( !isset( $val['name'] ) ): ?><?php echo Ibos::lang( 'No.step', '', array( '{step}' => $key ) ) ?><?php else: ?>[<?php echo $val['name']; ?>]<?php endif; ?> 
                                        <?php if ( !empty( $val['opuser'] ) ): ?><?php echo $lang['From']; ?> [<?php echo $val['opuser']; ?>] <?php echo $lang['Host work']; ?><?php endif; ?> 
                                        <?php if ( $val['flag'] == '2' ): ?> <strong class="fss lhl xco "><?php echo $lang['Perform']; ?></strong><?php endif; ?>
                                    </div>
                                    ...
                                    <?php elseif ( $val['flag'] == '2' ) : ?>
                                        <p class="fss lhl"><?php echo $lang['Continuous']; ?>：<?php echo $val['timestr']; ?>
                                            <?php if ( $val['timeoutflag'] ): ?><?php echo $lang['Timelimit'] . $val['timeout'] . $lang['Hour']; ?>,<?php echo $lang['Timeout'] . ':' . $val['timeused'] ?><?php endif; ?>
                                        </p>
                                    <?php endif; ?>
                                    <?php if ( $val['redo'] ): ?>
