    ...
    <div class="mcr">
        ...
        <?php if ( !empty( $flows ) ): ?>
            ...
            <?php foreach ( $flows as $catId => $flow ): ?>
                <?php $counter++; ?>
                <div class="wf-row <?php if ( $counter % 2 == 0 ): ?>bglg<?php endif; ?>">
...
<script>
    (function () {
        var threadId = '<?php echo intval( Env::getRequest( 'threadid' ) ); ?>';
        $("[data-toggle='tooltip']").tooltip();
        Ibos.events.add({
            'new': function (param, $elem) {
                var flowId = $elem.data('id'),
                    text = $elem.data('text'),
                    url = Ibos.app.url('workflow/new/add', {flowid: flowId,threadid:threadId}),
                        dialogOpt = {
                            id: 'd_start_work',
                            title: text,
                            width: '400px',
