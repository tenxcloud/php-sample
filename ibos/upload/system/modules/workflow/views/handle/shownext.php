			...
			<?php foreach ( $list as $key => $val ): ?>
				<?php if ( $val['isover'] ): ?>
					<?php if ( !empty( $prcsback ) ): ?>
						<div id="user_select_over" class="control-group" <?php if ( !$val['display'] ): ?>style="display:none;"<?php endif; ?>>
...
<?php if ( $notAllFinished != '' && $process['turnpriv'] == '0' ): ?>
				alert(Ibos.l('WF.AGENT') + '<?php echo $notAllFinished; ?>' + Ibos.l('WF.NOT_ALL_FINISHED_DESC'));
				return false;
<?php else: ?>
	<?php if ( $notAllFinished != '' && $process['turnpriv'] == '1' ): ?>
					msg += Ibos.l('WF.AGENT') + '[<?php echo $notAllFinished; ?>]' + Ibos.l('WF.NOT_ALL_FINISHED_DESC');
	<?php endif; ?>
				msg += Ibos.l('WF.CONFIRM_END_FLOW');
...
<?php endif; ?>
				...
				if (obj) {
					if (obj.checked) {
						...
						if (eval('document.turn_next_form.topflag' + i) && eval('document.turn_next_form.topflag' + i + '.value==0') && _id("prcs_user_op" + i).value == "") {
							Ui.tip(Ibos.l('WF.APPOINT_THE_HOST'), 'danger');
							return false;
						}
			...
			if (document.turn_next_form.prcs_choose.value == "") {
				Ui.tip(Ibos.l('WF.ATLEAST_SELECT_ONE_STEP'), 'danger');
				return false;
			}
<?php if ( $notAllFinished != '' && $process['turnpriv'] == '1' ): ?>
				msg = Ibos.l('WF.AGENT') + '[<?php echo $notAllFinished; ?>]' + Ibos.l('WF.NOTFINISHED_CONFIRM')
				if (!window.confirm(msg)) {
					return false;
				}
<?php elseif ( $notAllFinished != '' && $process['turnpriv'] == '0' ): ?>
				alert(Ibos.l('WF.AGENT') + '<?php echo $notAllFinished; ?>' + Ibos.l('WF.NOT_ALL_FINISHED_DESC'));
				return false;
<?php endif; ?>
		}//turn
