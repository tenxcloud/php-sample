	...
	<?php if ( !empty( $list ) ): ?>
		<ul class="content-ul">
			...
			<?php foreach ( $list as $catId => $flows ): ?>
				<?php $counter++; ?>
				<li <?php if ( $counter % 2 == 0 ): ?>class="bglg"<?php endif; ?>>
								...
								<li>
									<a class="wf-info-box" href="<?php echo Ibos::app()->createUrl( 'workflow/list/index', array( 'op' => 'list', 'type' => $type, 'sort' => $sort, 'flowid' => $flow['flowid'] ) ); ?>">
										<div class="classity-name">
											<?php echo $flow['name']; ?>
											<?php if ( isset( $flow['unreceive'] ) && !empty( $flow['unreceive'] ) ): ?><span class="badge prompt-info"><?php echo $flow['unreceive']; ?></span><?php endif; ?>
										</div>
