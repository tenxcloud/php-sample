...
CREATE TABLE `{{flow_process}}` (
  ...
  `overact` tinyint(1) NOT NULL DEFAULT '1' COMMENT '子流程结束后动作选择(0：结束并更新父流程节点为结束，1：结束并返回父流程步骤)，默认为1',
  PRIMARY KEY (`id`),
  KEY `flowid` (`flowid`),
...
CREATE TABLE `{{flow_form_type}}` (
  ...
  `printmodel` text NOT NULL COMMENT '表单设计信息',
  `printmodelshort` text NOT NULL COMMENT '精简后的表单设计信息',
  `deptid` text NOT NULL COMMENT '表单所属部门',
  `script` text NOT NULL COMMENT '表单拓展脚本',
  `css` text NOT NULL COMMENT '表单扩展样式',
...
INSERT INTO `{{nav}}`(`pid`, `name`, `url`, `targetnew`, `system`, `disabled`, `sort`, `module`) VALUES ('0','工作流','workflow/list/index','0','1','0','8','workflow');
