...
$(function() {
	...
	Ibos.evt.add({
		"saveForm": function() {
			...
			if ($.trim(content) === "") {
				alert(Ibos.l('WF.EMPTY_FORM_CONTENT'));
				return false;
			}
		...
		"previewForm": function() {
			window.open(Ibos.app.url('workflow/formtype/preview', {formid: Ibos.app.getPageParam("formid")}));
		},
		...
		"viewVersion": function() {
			var d = Ui.dialog({
				id: "d_version",
				title: Ibos.l("HISTORICAL_EDITION"),
				init: function() {
...
var IcMenu = function(options){
	...
	$footer = $('<div class="simple-editor-menu-footer">' +		
		'<button type="button" class="btn">' + Ibos.l("CANCEL") + '</button>' +
		'<button type="button" class="btn btn-primary pull-right">' + Ibos.l("SAVE") + '</button>' +
	'</div>');
...
(function(){
	...
	var icSettings = {
		"label": {
			id: "ic_label_menu",
			title: Ibos.l("WF.IC.LABEL"),
			ok: function(){
				var text = Dom.byId("ic_label_text").value;

				if($.trim(text) === "") {
					alert(Ibos.l("WF.ICTIP.PLEASE_INPUT_TEXT"));
					return false;
				}
		...
		"text": {
			...
			ok: function(){
				...
				if($.trim(title) === "") {
					alert(Ibos.l("WF.ICTIP.PLEASE_INPUT_TITLE"));
					return false;
				}
		...
		"select": {
			...
			ok: function(){
				...
				if($.trim(field) === "") {
					alert(Ibos.l("WF.ICTIP.PLEASE_INPUT_OPTIONS"));
					return false;
				}
		...
		"listview": {
			...
			ok: function(){
				...
				if(!lvTitles.length) {
					alert(Ibos.l("WF.ICTIP.PLEASE_INPUT_COLUMN_DATA"));
					return false;
				}
		...
		"progressbar": {
			...
			title: Ibos.l("WF.IC.PROGRESSBAR"),
		...
		"qrcode": {
			...
			ok: function(){
				...
				if ($.trim(value) === "") {
					alert(Ibos.l("WF.ICTIP.PLEASE_INPUT_QRCODE_VALUE"));
					return false;
				}
