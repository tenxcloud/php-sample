...
Wfs.formEdition = (function() {
	var $editionDialog = $("#dialog_edition"),
			...
			_createOptionTpl = function(datas) {
				var tpl = "<option value='0'>" + Ibos.l("WF.SELECT_HISTORICAL_EDITION") + "</option>";
				if (datas.length) {
			...
			show = function(param) {
				...
				var d = Ui.dialog({
					...
					title: Ibos.l("WF.HISTORICAL_EDITION"),
					cancel: function() {
						_fid = void(0);
					},
			...
			restore = function(fid, version) {
				if (fid && version && version !== "0") {
					Ui.confirm(Ibos.l("WF.EDITION_RESTORE_CONFIRM"), function() {
			...
			del = function(fid, version) {
				if (fid && version && version !== "0") {
					Ui.confirm(Ibos.l("WF.EDITION_DELETE_CONFIRM"), function() {
...
Wfs.formItem = (function() {
	var dialogNode = document.getElementById("dialog_form_setting"),
			...
			add = function(param) {
				...
				Ui.dialog({
					...
					button: [{
							...
							name: Ibos.l('WF.SAVE_AND_DESIGN'),
							callback: function() {
								if ($.trim($name.val()) == '') {
									$name.blink();
									return false;
			...
			_save = function(param, dom) {
				$(formNode).waiting(Ibos.l('IN_SUBMIT'), "mini", true);
...
Wfs.formList = (function() {
	...
	var removeItem = function(ids, param) {
		if (ids) {
			Ui.confirm(Ibos.l('WF.CONFIRM_DEL_FORM'), function() {
