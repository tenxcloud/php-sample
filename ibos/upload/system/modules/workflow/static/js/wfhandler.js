 ...
 * IBOS
 * @author		banyan
 * @version	$Id: wfhandler.js 3997 2014-08-26 08:53:45Z gzzcs $
 */
...
$(function() {
	...
	Ibos.events.add({
		...
		'end': function(param, $ctx) {
			Ui.confirm(WF.CONFIRM_END_FREE_PROCESS, function() {
		...
		'finish': function(param, $ctx) {
			...
			if (Ibos.app.g('feedback') === FORCE_FEEDBACK) {
				if ($.trim(getForm(true).content.value) === '') {
					Ui.tip(WF.FORCED_SIGN_OPINION, 'danger');
					$(getForm(true).content).blink();
					return false;
				}
			}
			Ui.confirm(WF.CONFIRM_FINISH_WORK, function() {
		...
		'cancel': function(param, $ctx) {
			Ui.confirm(WF.CONFIRM_UNSAVE_WORK, function() {
		...
		'delFb': function(param, $ctx) {
			Ui.confirm(WF.CONFIRM_DEL_FB, function() {
		...
		'fallback': function(param, $ctx) {
			// 回退上一步骤
			if (param.type == 1) {
				Ui.prompt(WF.ENTER_FALLBACK_REASON, function(msg) {
					if (msg === '') {
						Ui.tip(WF.ENTER_FALLBACK_REASON, 'danger');
						return false;
					}
					var form = getForm();
				...
				Ui.dialog({
					...
					ok: function() {
						...
						if (url) {
							...
							Ui.alert(WF.SELECT_FALLBACK_STEP);
						}
						return false;
					}
				});
