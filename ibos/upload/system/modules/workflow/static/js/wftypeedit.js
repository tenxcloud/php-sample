(function() {
	...
	$("#flow_name").formValidator().regexValidator({
		dataType: "enum",
		regExp: "notempty",
		onError: Ibos.l("WF.FLOW_NAME_CANNOT_BE_EMPTY")
	});
	$('#form_name').formValidator().functionValidator({fun: checkFormName});
	function checkFormName(val) {
		if ($('#form_select').val() === '' && val === '') {
			return Ibos.l("WF.ENTER_FORM_NAME");
		}
		return true;
	}

	$('#flow_catid').formValidator({
		onShow: Ibos.l("WF.SELECT_FLOW_TYPE"),
		onFocus: Ibos.l("WF.SELECT_FLOW_TYPE")
	}).inputValidator({
            min: 0, 
            onError: Ibos.l("WF.SELECT_FLOW_TYPE")
        });
	...
	// 使用状态说明
	$('#using_state').popover({html: true, title: Ibos.l("WF.USING_STATE_TITLE"), content: Ibos.l("WF.USING_STATE_DESC"), trigger: 'hover'});
	// 委托类型说明
	$('#delegate_type').popover({html: true, title: Ibos.l("WF.DELEGATE_TYPE_TITLE"), content: Ibos.l('WF.DELEGATE_TYPE_DESC'), trigger: 'hover'});

	// 预览表单
	$formPreviewBtn.on("click", function() {
		if (formId) {
			window.open(Ibos.app.url('workflow/formtype/preview', {formid: formId}), "preview");
		}
	});
	...
	Ibos.checkFormChange("#type_form", Ibos.l("WF.TYPE_FORM_CHANGE_TIP"));
})();