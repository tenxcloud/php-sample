	...
	FormCondition.prototype = {
		...
		addLeftParenthesis: function(logic) {
			...
			if (len > 0) {
				...
				//有条件才能添加左括号表达式
				Ui.tip("@WF.CONDITION_NEED", "warning");
				return;
			}
			...
			if (($.trim(text).substr(-3, 3) === 'AND') || ($.trim(text).substr(-2, 2) === 'OR')) {
				Ui.tip("@WF.CONDITION_CANNOT_EDIT", "warning");
				return;
			}
			if (text.indexOf('(') >= 0) { //检查括号匹配
				if (!this._valid(text)) {
					Ui.tip("@WF.CONDITION_FORMAT_ERROR", "warning");
					return;
				} else {
					text += " " + logic;
					value += " " + logic;
		...
		addCondition: function(field, operator, value, logic, textValue) {
			...
			if (isNotNullString(field) && isNotNullString(operator) && isNotNullString(value)) {
				...
				if (value.indexOf("'") >= 0) {
					Ui.tip("@WF.CONDITION_INVAILD", "warning");
					return;
				}
				...
				for (var i = 0; i < len; i++) {
					if (this.node.options[i].text.indexOf(newText) >= 0) {
						Ui.tip("@WF.CONDITION_REPEAT", "warning");
						return;
					}
				}
				...
				Ui.tip("@WF.CONDITION_INCOMPLETE", "warning");
				return;
			}
		},
...
var QueryForm = {
	...
	dateChange: function(val, $dateBegin, $dateEnd) {
		...
		var dbdp = $dateBegin.data("datetimepicker"),
			dedp = $dateEnd.data("datetimepicker");

		dbdp.setDate(dbdp.parseDate(date.begin));
		dedp.setDate(dedp.parseDate(date.end));
	}
}
...
$(function() {
	...
	$("#turn_hidden").on("click", function() {
		...
		if (selectedItem.length == 1 && $("#show_select option[value='" + groupField + "']").is(":selected")) {
			Ui.tip(Ibos.l("WF.GROUPING_FIELD_CAN_NOT_HIDE"), 'warning');
		}
	...
	$("#save_prepare_project").on("click", function() {
		Ui.prompt(Ibos.l('WF.PLEASE_ENTER_THE_PACKAGE_NAME'), function(val) {
			if ($.trim(val) == '') {
				return false;
			}
			$('#tplname').val(val);
			$('#query_form').attr('action', Ibos.app.url('workflow/query/add')).submit();
		}, Ibos.l('WF.NEW_PLAN_EXAMPLE'));
	});
	/**
	 * 初始化流程开始时间范围显示
	 */
