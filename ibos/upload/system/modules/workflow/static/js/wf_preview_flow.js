...
$(function() {
	...
	Ibos.events.add({
		...
		'redo': function(param, $elem) {
			Ui.confirm(Ibos.l('WF.CONFIRM_REDO'), function() {
				...
				$.post(url, {
					...
					if (data.isSuccess) {
						...
						Ui.tip(Ibos.l('WF.OPERATION_FAILED'), 'danger');
					}
				}, 'json');
			});
		}
	...
	$(document).ready(function() {
		if (U.getCookie('workflow_todo_remind_' + Ibos.app.g("runId")) == '1') {
			$('#todo_remind_btn').addClass('disabled').html(Ibos.l('WF.ALREADY_SEND_REMIND'));
		} else {
			Ibos.events.add({ 
				// 超时催办
				"overtimeRemind": function(param, $elem) {
					Ui.dialog({
						id: 'd_confirm_remind',
						title: Ibos.l('WF.SEND_TODO_REMIND'),
						padding: 0,
						...
						ok: function() {
							...
							$('#confirm_remind').waiting(Ibos.l('IN_SUBMIT'), "mini");
							...
							$.post(url, {
								...
								if (data.isSuccess) {
									...
									Ui.tip(Ibos.l('OPERATION_SUCCESS'), 'danger');
								}
							}, 'json');
							return false;
						},
