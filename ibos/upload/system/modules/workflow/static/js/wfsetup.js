...
var searchTemplate = {
	...
	showList: function(param, ok, cancel) {
		...
		guide.dialog(this.listUrl, param, {
			title: Ibos.l('WF.FLOW_QUERY_TPL'),
			ok: false,
			cancel: true
		});
	},
	showSetting: function(param, ok, cancel) {
		...
		if (param) {
			url = this.editUrl;
			title = Ibos.l('WF.EDIT_QUERY_TPL');
		} else {
			url = this.addUrl;
			title = Ibos.l('WF.NEW_QUERY_TPL');
		}
		guide.dialog(url, param, {
			...
			ok: function() {
				...
				if ($conditionSelect.length > 0) {
					...
					if (!checkExp) {
						Ui.tip(Ibos.l('WF.CONDITION_FORMAT_ERROR'), 'danger');
						if (tab) {
				...
				$form.waiting(Ibos.l('IN_SUBMIT'), "mini", true);
				$.post(url, $form.serializeArray(), function(data) {
					$form.stopWaiting();
					if (data.isSuccess) {
						Ui.tip(Ibos.l('SAVE_SUCEESS'), 'success');
						that.showList();
					} else {
						Ui.tip(Ibos.l('SAVE_FAILED'), 'danger');
					}
				}, 'json');
				return false;
			},
...
var Manager = {
	...
	showSetup: function(param) {
		...
		if (param) {
			url = this.EditUrl;
			title = Ibos.l('WF.EDIT_MANAGE_PRIV');
		} else {
			url = this.AddUrl;
			title = Ibos.l('WF.NEW_MANAGE_PRIV');
		}
		guide.dialog(url, param, {
			title: title,
			ok: function() {
				if ($.trim($('#auth_for').val()) === '') {
					Ui.tip(Ibos.l('WF.INVALID_AUTH_OBJECT'), 'danger');
					return false;
				}
				if ($('#manage_scope').val() === 'custom' && $('#custom_department_select').val() === '') {
					Ui.tip(Ibos.l('WF.INVALID_CUSTOM_DEPT'), 'danger');
					return false;
				}
				$('#auth_rule_setting').waiting(Ibos.l('IN_SUBMIT'), "mini", true);
...
var timer = {
	...
	showList: function(param, ok, cancel) {
		var that = this;
		guide.dialog(this.listUrl, param, {
			title: Ibos.l('WF.SET_THE_TIMING_TASK'),
			ok: function() {
				$('#crontab_form').waiting(Ibos.l('IN_SUBMIT'), "mini", true);
...
var free = {
	...
	show: function(param) {
		...
		guide.dialog(this.url, param, {
			id: 'free_new_user',
			title: Ibos.l('WF.SPECIFY_FREE_FLOW_NEW_USER'),
			ok: function() {
	...
	var handler = {
		...
		importFlow: function(flowid) {
			Ui.openFrame(Ibos.app.url('workflow/type/import', {flowid: flowid}), {
				id: 'import_frame',
				title: Ibos.l('WF.SPECIFY_THE_IMPORT_FILE'),
				padding: 20,
				ok: function() {
					var ifm = this.iframe;
		...
		designForm: function() {
			window.open(Ibos.app.url('workflow/formtype/design', {formid: guide.getFormId()}), "designForm");
		},
		previewForm: function() {
			window.open(Ibos.app.url('workflow/formtype/preview', {formid: guide.getFormId()}), "previewForm");
		},
		designFlow: function(flowid) {
			window.open(Ibos.app.url('workflow/process/index', {flowid: flowid}), "designFlow");
		},
		verifyFlow: function(flowid) {
			var opt = {
				title: Ibos.l('WF.EXAM_FLOW'),
				id: 'd_verify',
				padding: 0,
				cancel: true
			};
