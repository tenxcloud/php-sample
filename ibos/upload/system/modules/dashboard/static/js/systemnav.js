var navigation = {
	...
	collapseChildNav : function($elem, state){
		...
		state === "start" ? $childList.hide() : $childList.show();
	},
		...
		switch(act){
			...
			case "add_sec":
				var d = new Date(),
				 	...
				 	liType = "child",
				 	newNav = $.template('new_nav', {id: d.getTime(), pid: pid});
				navigation.addLine(target, newNav, liType, function($li){
					$li.find("input[type=checkbox]").label();
					navigation.initMainNavBackground($mianLi);
					$li.attr("data-id", d.getTime());
