	...
	<div>
		...
		<div class="ctb">
			...
			<form action="<?php echo $this->createUrl( 'credit/rule' ); ?>" method="post" class="form-horizontal">
					...
					<tbody>
						<?php foreach ( $rules as $rule ) : ?>
							<tr>
								...
								<td>
									<select data-type="cycletype" data-value="<?php echo $rule['rid']; ?>" name="cycles[<?php echo $rule['rid']; ?>]" class="input-small">
										<option value="1" <?php if ( $rule['cycletype'] == '1' ): ?>selected<?php endif; ?>><?php echo $lang['Once']; ?></option>
										<option value="2" <?php if ( $rule['cycletype'] == '2' ): ?>selected<?php endif; ?>><?php echo $lang['Per hour']; ?></option>
										<option value="3" <?php if ( $rule['cycletype'] == '3' ): ?>selected<?php endif; ?>><?php echo $lang['Every day']; ?></option>
										<option value="4" <?php if ( $rule['cycletype'] == '4' ): ?>selected<?php endif; ?>><?php echo $lang['Weekly']; ?></option>
										<option value="5" <?php if ( $rule['cycletype'] == '5' ): ?>selected<?php endif; ?>><?php echo $lang['Per mensem']; ?></option>
									</select>
								</td>
								<td>
									<input id="reward_num_<?php echo $rule['rid']; ?>" type="text" value="<?php echo $rule['rewardnum']; ?>" name="rewardnums[<?php echo $rule['rid']; ?>]" class="input-small">
								</td>
								<?php foreach ( $credits as $index => $credit ) : ?>
									<?php $offset = (integer) ($index + 1); ?>
									<td>
										<input type="text" value="<?php echo $rule['extcredits' . $offset] ?>" length="4" name="credits[<?php echo $rule['rid']; ?>][<?php echo $offset; ?>]" class="input-small">
									</td>
								<?php endforeach; ?>
							</tr>
						<?php endforeach; ?>
				...
				<button name="creditRuleSubmit" class="btn btn-primary btn-large btn-submit" type="submit"><?php echo $lang['Submit']; ?></button>
			</form>
		</div>
	</div>
</div>
